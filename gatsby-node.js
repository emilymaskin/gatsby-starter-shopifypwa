...
exports.createPages = async ({ graphql, actions }) => {
  ...
  const getMoreProducts = async function(currentCursor) {
    ...
    allProducts = allProducts.concat(
      productsCache.data.shopify.shop.products.edges,
    );
    ...
    if (productsCache.data.shopify.shop.products.pageInfo.hasNextPage) {
      await getMoreProducts(
        (currentCursor =
          productsCache.data.shopify.shop.products.edges[
            productsCache.data.shopify.shop.products.edges.length - 1
          ].cursor),
      );
    }
  };
  ...
  allCollections &&
    allCollections.data.shopify.shop.collections.edges.forEach(edge => {
      createPage({
